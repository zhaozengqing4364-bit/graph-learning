...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install the Codex safe-grow workflow into another project."
    )
...
def main() -> int:
    ...
    template_root = script_dir.parent / "templates" / "codex-safe-grow"
    ...
    for destination, rendered in planned_writes:
        ...
        if destination.name == "codex-loop.sh":
    ...
    if planned_writes:
        print("")
        print("Next steps:")
        print(f"1. Fill {target_root / '.codex/loop/PROJECT_GROWTH.md'} with project-specific guidance.")
        print(f"2. Paste audit results into {target_root / '.codex/loop/GLM_AUDIT.md'}.")
        print("3. Use $safe-grow interactively, or run scripts/codex-loop.sh for one automated iteration.")
    else:
